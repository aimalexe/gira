        ...
        <div className="p-4 border rounded-md bg-gray-100">
            {isEditing ? (
                ...
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center">
                    <div>
                        <h3 className="text-lg font-medium text-indigo-800">{task.title}</h3>
                        <p className="text-gray-600">{task.description}</p>
                        <p className="text-gray-600">Status: {task.status}</p>
                        <p className="text-gray-600">Assigned To: {users.find(u => u._id === task.assignedTo)?.name || 'Unassigned'}</p>
                        <p className="text-gray-600">Due: {new Date(task.dueDate).toLocaleDateString()}</p>
                        {task.fileAttachment && (
                            <a href={task.fileAttachment} target="_blank" rel="noopener noreferrer" className="text-teal-500 underline">
                                View Attachment
                            </a>
                        )}
                    </div>
